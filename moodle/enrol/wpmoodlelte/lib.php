...
function enrol_users_via_wpmoodlelte ()
{
    ...
    try{
        $invoices_sql = "SELECT * FROM {enrol_wpmoodlelte} where paid_status = 'Paid' and deleted_at is NULL";
        $invoices = $DB->get_records_sql($invoices_sql);
        ...
        foreach($invoices as $row){
            if ($row->paid_status == 'Paid') {
                ...
                if($row->user_id != "0"){
                    ...
                    $coursecontext = \context_course::instance($row->course_id);
                    
                    if(!is_enrolled($coursecontext, $row->user_id)){
                        $plugin_instance = $DB->get_record("enrol", array('id' => $row->enrol_id, 'enrol' => 'wpmoodlelte'));
                        $plugin = enrol_get_plugin('wpmoodlelte');
                        $plugin->enrol_user($plugin_instance, $row->user_id, 5);
                    ...
                    $user_data = $DB->get_record("user", array('email' => $row->email ));
                    ...
                    $DB->execute("Update {enrol_wpmoodlelte} set paid_status='" . $row->paid_status . "', user_id= ".$userid."  where invoice_id='" . ($row->invoice_id)."'");
                    $coursecontext = \context_course::instance($row->course_id);
...
function enrol_users_via_wpmoodlelte_2 ()
{
    global $DB, $CFG;
    ...
    try{
        ...
        for($i=0; $i < ceil($total_records / $limit ); $i++){
            ...
            try{
                $sql = "SELECT GROUP_CONCAT(c_id SEPARATOR ',') AS challan_ids from (SELECT invoice_id as c_id FROM {enrol_wpmoodlelte} where paid_status = 'Not Paid' and deleted_at is NULL LIMIT ".($limit*$i).", $limit) as c_tab";
...
class enrol_wpmoodlelte_plugin extends enrol_plugin {
    ...
    public function create_new_wpmoodlelte($course_id, $name, $email, $amount, $mobile = '', $invoice_id, $status, $date, $download_invoice ){
        global $CFG, $DB;
        ...
        $response['data'] = [
            ...
            'total_amount' => $amount,
            'paid_status' => $status,
            'url_for_download_wpmoodlelte' => $download_invoice
    ...
    public function insertwpmoodlelte($data, $course_id, $user_id, $enrol_id){
        try {
            ...
            if ($data) {
                ...
                $dataobj->invoice_id = $data['invoice_id'];
                $dataobj->due_date = $data['due_date'];
                $dataobj->created_date = $data['due_date'];
                ...
                $dataobj->url_for_download_wpmoodlelte = $data['url_for_download_wpmoodlelte'];
                ...
                $dataItem->title = $data['name'];
                $dataItem->course_id = $course_id;
                $dataItem->amount = $data['total_amount'];
                $enrol_wpmoodlelte = $DB->insert_record('enrol_wpmoodlelte_items', $dataItem);
                
                //Email wpmoodlelte to User 
                //$this->send_wpmoodlelte_email($dataobj->name, $dataobj->email, $dataobj->url_for_online_payment, $dataobj->url_for_download_wpmoodlelte, $dataobj->invoice_id, $course->fullname);
